...
const renderTree = (treeNode: Object, prop: string, render: Function, level = 0) => {
  ...
  render() {
    ...
    return (
      <ul className={cx(styles.tree, genericStyles.scrollbars, className, 'meta-test__enterName')}>
        {operationObject === '' && fileOperation && ['createFile', 'createFolder'].includes(fileOperation) && (
          <NewTreeElement
            filePaths={filePaths}
          ...
          renderTree(x, 'items', (item, children, level) => {
            ...
            return fileOperation === 'rename' && operationObject === item.path ? (
              ...
              <FileTreeElement
                ...
                onRename={onRename}
              >
                {operationObject === item.path &&
                  fileOperation &&
                  ['createFile', 'createFolder'].includes(fileOperation) && (
                    <NewTreeElement
                      active={selectedFile ? selectedFile.path === item.path : false}
                      filePaths={filePaths}
                      level={level + 1}
